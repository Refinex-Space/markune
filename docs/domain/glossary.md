...
- Drawing back-link: a stable `markune-drawing://{drawing-id}` link that opens the source drawing without depending on its title or album path.
...
- Capture: one lightweight Markdown fragment stored under `.markune/inbox`, identified by its file name and carrying triage metadata in camelCase frontmatter.
...
- Git Sync: workspace-level Git automation for committing local changes, pulling remote updates, pushing to the configured remote, and recording sync preferences in `.markune/workspace.json`.
