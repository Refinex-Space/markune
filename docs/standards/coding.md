...
- Large-document media resolution must use the editor-level `resolveMediaSource` bridge and one batched workspace IPC. Mount the Markdown body before that IPC completes, share bounded positive/negative/in-flight results across Tab remounts, persist `madora-asset://` in nodes, and never project resolved file URLs through a whole-document string replacement or DOM-driven editor transaction.
