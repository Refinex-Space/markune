...
必须在分配过程中限制文档、目录条目、层级、关系、辅助节点及投影大小，而不是构建完成后截断；原生图谱任务全局串行。YAML 在 loader 前检查输入、深度、事件数和别名展开成本，拒绝递归/爆炸式展开。元数据与链接只作为数据解析，不能执行表达式、访问网络或读取链接指定的文件；目标只能来自扫描索引。前端仅收到相对路径和图谱投影，不返回正文与任意属性值。
