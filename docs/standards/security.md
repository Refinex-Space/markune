...
- 内置 AI 画图 Skill 根目录只能由 Tauri resource resolver 取得并经 Rust 注入；渲染器提交任意 `extraRoots` 或 `dynamicTools` 必须失败关闭。动态工具请求必须精确匹配 `madora_drawing` namespace、允许的工具名和字段，未知字段、超长 Mermaid、非 UUID previewId/drawingId、重复响应、外链 Data URL 和伪造图片签名一律拒绝。`inspect_drawing` 只能读取当前 turn 的 active/mention 授权集合，授权在 turn 完成、运行时退出、工作区切换或下一 turn 时撤销。
