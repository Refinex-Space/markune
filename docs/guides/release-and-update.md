...
当前发布目标为 `0.2.5`，配套编辑器为 `markweave@0.10.4` 与 `@markweave/react@0.10.4`。本次说明涵盖工作区自动刷新、附件存储、元数据与链接保真、图谱和知识整理、Codex 问答及编辑体验。Tag 前同步 `.github/workflows/release.yml` 的 `releaseBody` 与官网 `markune-web` 的 `src/content/changelog.ts`；官网发布日期以正式 Release 返回的时间为准。
